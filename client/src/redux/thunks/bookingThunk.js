...
export const bookCar = createAsyncThunk('bookings/bookCar', async (reqObj, { rejectWithValue }) => {
    try {
      await axios.post('https://drivos.onrender.com/api/bookings/bookcar', reqObj);
      return reqObj;
    } catch (error) {
  ...
  export const getAllBookings = createAsyncThunk('bookings/getAllBookings', async (_, { rejectWithValue }) => {
    try {
      const response = await axios.get('https://drivos.onrender.com/api/bookings/getallbookings');
